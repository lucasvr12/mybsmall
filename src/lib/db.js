...
// A helper to execute multiple queries sequentially on a single client connection.
// This guarantees that exactly one connection is used per request block, preventing connection slot exhaustion
// and speeding up performance by avoiding repeated TCP and SSL handshakes.
...
// Mock the @vercel/postgres template literal query interface.
export const pool = {
  async sql(strings, ...values) {
    return withClient(sql => sql(strings, ...values));
  }
};
